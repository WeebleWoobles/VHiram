...
void timing_test_task(void* pvParameters) {
    ...
    while (1) {
        ...
        // ---- Get Pitch Estimate (fused accel + gyro) ----
        prev_pitch = pitch;
        pitch = PID_get_pitch(pitch, dt);

        if (pitch != prev_pitch) {  // Detect IMU update based on pitch change
            imu_count++;
            if (imu_count % 100 == 0) {
        ...
        // ---- PID Correction and Motor Command ----
        ...
        MOTOR_set_speed(-motor_cmd);  // Negative for direction adjustment
